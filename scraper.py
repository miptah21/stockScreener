...
try:
    from ojk_scraper import get_bank_ratios
except ImportError:
...
def get_financials(ticker_symbol: str, target_year: int = None, freq: str = 'annual') -> dict:
    ...
    print(f"[Fallback] Yahoo Finance: success={yahoo_ok}, completeness={yahoo_completeness:.1%}")
    ...
    if not (yahoo_ok and yahoo_completeness >= 0.5):
         try:
            ...
            print(f"[Fallback] Alternative ({alt_source}): success={alt_ok}, completeness={alt_completeness:.1%}")
            ...
            print(f"[Fallback] Alternative sources failed: {e}")
            if not yahoo_ok:
    ...
    if result.get('success') and result.get('data'):
        ...
        if fin_subsector == 'bank' and get_bank_ratios:
            print(f"[Scraper] Checking OJK data for {ticker_symbol}...")
            for year_data in result['data']:
                ...
                if y_val:
                    try:
                        ...
                        if ojk_data:
                            ...
                            print(f"[Scraper] Injected OJK data for {ticker_symbol} FY{y_val}")
                    except Exception as e:
                        print(f"[Scraper] Error fetching/injecting OJK data: {e}")
        ...
        if target_year:
            ...
            if idx != -1:
                ...
                print(f"[Scraper] Time Machine: Analysis Year {target_year} (Index {idx})")
            else:
                print(f"[Scraper] Target year {target_year} not found. Using latest available data.")
