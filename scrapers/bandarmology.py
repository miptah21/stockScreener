...
def _fetch_single_day(ticker, date):
    ...
    k1 = Config.GOAPI_API_KEY
    k2 = Config.GOAPI_API_KEY_2
    keys = [k for k in [k1, k2] if k]
    
    if not keys:
        logger.error("No GoAPI keys configured")
        return []
    ...
    for i, key in enumerate(keys):
        ...
        logger.debug("Trying Key %d (%s) for %s", i + 1, mask, date)
        ...
        for url in endpoints:
            try:
                ...
                if response.status_code == 429:
                    logger.warning("Key %d rate limited (429)", i + 1)
                ...
                if response.status_code == 200:
                    ...
                    if status == 'success' and data.get('data'):
                        logger.info("Key %d success for %s", i + 1, date)
                        return data['data'].get('results', [])
                    else:
                        logger.debug("Key %d returned status: %s", i + 1, status)
                        
            except Exception as e:
                logger.debug("Key %d error: %s", i + 1, e)
                pass
                
    if last_error:
        logger.warning("All GoAPI keys failed/rate limited")
        return last_error
