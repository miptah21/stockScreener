...
def classify_rsi(rsi_value: float, prev_rsi: float = None) -> dict:
    ...
    if rsi_value < 30:
        ...
        return {'zone': 'bullish', 'label': 'Bullish', 'color': 'buy'}
    else:
        return {'zone': 'overbought', 'label': 'Overbought', 'color': 'sell'}


# ─── MACD Calculation ────────────────────────────────────────────────
def calculate_macd(closes: pd.Series, fast: int = 12,
                   slow: int = 26, signal: int = 9) -> dict:
    """
    Calculate MACD, Signal Line, and Histogram.

    Returns:
        dict with 'macd_line', 'signal_line', 'histogram' as pd.Series
    """
...
def detect_macd_crossover(macd_line: pd.Series,
                          ...
                          lookback: int = 3) -> dict:
    """
    Detect recent MACD/Signal crossover within lookback period.

    Returns:
        dict with crossover type, label, color, and bars_ago
    """
    ...
    for i in range(1, min(lookback + 1, len(diff))):
        ...
        curr_diff = diff.iloc[idx]
        prev_diff = diff.iloc[prev_idx]
